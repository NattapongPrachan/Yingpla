...
public class AreaManager : MonoSingleton<AreaManager>
{
    ...
    public static Vector2 GetOuterBounds()
    {
        ...
        {
            case 0: // ซ้าย
                randomPosition = new Vector2(Instance.AreaBounds.min.x - Instance.SpawnOffsset, Random.Range(Instance.AreaBounds.min.y, Instance.AreaBounds.max.y));
                break;
            case 1: // ขวา
                randomPosition = new Vector2(Instance.AreaBounds.max.x + Instance.SpawnOffsset, Random.Range(Instance.AreaBounds.min.y, Instance.AreaBounds.max.y));
                break;
            case 2: // บน
                randomPosition = new Vector2(Random.Range(Instance.AreaBounds.min.x, Instance.AreaBounds.max.x), Instance.AreaBounds.max.y + Instance.SpawnOffsset);
                break;
            case 3: // ล่าง
                randomPosition = new Vector2(Random.Range(Instance.AreaBounds.min.x, Instance.AreaBounds.max.x), Instance.AreaBounds.min.y - Instance.SpawnOffsset);
                break;
        }
